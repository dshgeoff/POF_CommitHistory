...
public class StringRequest extends BaseRequest<POFResponse, StringRequest> {
    ...
    public StringRequest(
            ...
            Response.Listener<POFResponse> listener,
    ...
    @Override
    protected Response<POFResponse> parseNetworkResponse(NetworkResponse response) {
        String json = "";
        ...
        POFResponse respond = new POFResponse();
        respond.header = response.headers;
        respond.json = json;
        try {
            respond.setPOFError(parseXML(response.headers.get("message")));
        } catch (Exception e) {
        }
